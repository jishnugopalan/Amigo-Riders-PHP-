...
function getResult(tid) {
  ...
  xhttp.open("GET", "getEvent.php?q="+tid, true);
  xhttp.send();
}
...
$sql="SELECT * FROM event_reg a,event b where a.email='$id' and a.event_id=b.event_id and b.admin_s='y'";
$result=mysqli_query($conn,$sql);
if(mysqli_num_rows($result)>0)
	
{
?>
<label for="sel1">Select an event</label>

			<center>

				 <select class="form-control" name="ename" id="country-list" class="demoInputBox" onChange="getResult(this.value);">
...
echo "<center><h1>No events are available</h1></center>";
