...
<h4><font color="blue">Here you can edit your trip.The edited trip will be send to the admin for approval.The trip will be displayed after the approval.You shoult edit your trip one day before the starting date.Otherwise the trip will be not displayed</font></h4>
</div>
</div></div>
...
$sql="SELECT * FROM trip where email='$id'";
$result=mysqli_query($conn,$sql);
if (mysqli_num_rows($result) > 0) {
 while($row = mysqli_fetch_assoc($result)) {
 	$bid=$row["trip_id"];
...
<h3><?php echo $des ?></h3><br>
<a href="trip.php?sid=<?php echo $bid ?>"><button class="btn btn-success">Update</button></a>
</div>
...
echo "<h3>No trips are availabe</h3>";
?>
