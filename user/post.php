...
<body>
	...
	<?php
include "dbconn.php";
$sql="SELECT gallery.item_id,gallery.uploaded_on,gallery.url,gallery.item_type,u_r_reg.name,u_r_reg.photo FROM gallery INNER JOIN u_r_reg ON gallery.email=u_r_reg.email order by gallery.uploaded_on DESC";
          ...
          else
          {
          	...
          	echo "<video controls  width='500' height='300'><source src='$video' type='video/mp4' />";
										...
										<ul>
											...
											<?php
											$sql2="SELECT COUNT(*) FROM likes where item_id='$bid' and item_name='post'";
          ...
          if(mysqli_num_rows($result2) > 0)
          {
            while($row2=mysqli_fetch_assoc($result2))
            {
            	$n=$row2["COUNT(*)"];
											...
											?>
											<?php echo $n;?>likes
											<li><a href="postlike.php?id=<?php echo $bid ?>"><button><i class="" aria-hidden="true"></i>like</button></a></li>
